...
internal static partial class OpenaiImagesEdits2CommandApiCommand
{
    ...
    private static readonly OpenAIImagesEditsInOptionSet InpOptions = OpenAIImagesEditsInOptionSet.Create(@"inp");
    ...
    {
        var command = new Command(@"openai-images-edits2", @"Openai Images Edits
Edit image using OpenAI Images Edits API");
                        ...
                        command.Options.Add(InpOptions.Prompt);
                        command.Options.Add(InpOptions.Maskname);
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            ...
            {
                        var __requestBase = await CliRuntime.ReadRequestOrDefaultAsync<global::DeepInfra.BodyOpenaiImagesEditsV1OpenaiImagesEditsPost>(
                            parseResult,
                            Input,
                            RequestJson,
                            RequestFile,
                        ...
                        var inpPrompt = parseResult.GetValue(InpOptions.Prompt);
                        var inpMaskname = CliRuntime.WasSpecified(parseResult, InpOptions.Maskname) ? parseResult.GetValue(InpOptions.Maskname) : (__InpBase is { } __InpmasknameBaseValue ? __InpmasknameBaseValue.Maskname : default);
                        ...
                        var inp =
                            __InpSpecified || __InpBase is not null
                                ? new global::DeepInfra.OpenAIImagesEditsIn
                                {
	                                Model = inpModel!,
                                N = inpN,
                                Size = inpSize,
                ...
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);
                                ...
                                var response = await client.OpenaiImagesEdits2Async(
                                    xiApiKey: xiApiKey,
                                    image: image,
                                    imagename: imagename,
                                    prompt: prompt,
