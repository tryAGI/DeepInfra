...
internal static partial class ModelsModelMetaUpdateCommandApiCommand
{
    ...
    private static Option<string?> SampleOutputUrl { get; } = new(
        name: @"--sample-output-url")
    {
        Description = @"dataurl or regular url to a sample of this model's output, shown on the model page before the first run (empty to delete)",
    };
    ...
    public static Command Create()
    {
        var command = new Command(@"meta-update", @"Model Meta Update");
                        ...
                        command.Options.Add(SampleOutputUrl);
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            await CliRuntime.RunAsync(async () =>
            {
                        ...
                        var sampleOutputUrl = CliRuntime.WasSpecified(parseResult, SampleOutputUrl) ? parseResult.GetValue(SampleOutputUrl) : (__requestBase is { } __SampleOutputUrlBaseValue ? __SampleOutputUrlBaseValue.SampleOutputUrl : default);
                ...
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);
                                ...
                                var response = await client.Models.ModelMetaUpdateAsync(
                                    modelName: modelName,
                                    xiApiKey: xiApiKey,
                                    xApiKey: xApiKey,
