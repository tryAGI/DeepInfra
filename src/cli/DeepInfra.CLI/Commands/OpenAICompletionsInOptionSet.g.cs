...
internal sealed record OpenAICompletionsInOptionSet(
    Option<string> Model,
                     ...
                     Option<string?> User,
                     Option<int?> Seed,
                     Option<bool?> ReturnTokensAsTokenIds,
                     Option<string?> PromptCacheKey)
{
    public static OpenAICompletionsInOptionSet Create(string? prefix = null)
    {
        var normalizedPrefix = string.IsNullOrWhiteSpace(prefix)
            ? string.Empty
            : prefix.Trim().Trim('-') + "-";
        return new OpenAICompletionsInOptionSet(
...
The total length of input tokens and generated tokens is limited by the model's context length.If explicitly set to None it will be the model's max context length minus input length or 16384, whichever is smaller.",
                ...
                Logprobs: new Option<int?>($"--{normalizedPrefix}logprobs")
                {
                    Description = @"return top tokens and their log-probabilities",
                },
                Echo: CliRuntime.CreateNullableBoolOption(name: $"--{normalizedPrefix}echo", description: @"return prompt as part of the respons"),
                ...
                ReturnTokensAsTokenIds: CliRuntime.CreateNullableBoolOption(name: $"--{normalizedPrefix}return-tokens-as-token-ids", description: @"return tokens as token ids"),
