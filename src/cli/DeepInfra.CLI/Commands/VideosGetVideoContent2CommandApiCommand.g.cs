...
internal static partial class VideosGetVideoContent2CommandApiCommand
{
    ...
    private static Option<string?> Variant { get; } = new(
        name: @"--variant")
    {
        Description = @"",
    };
    ...
    {
        var command = new Command(@"get-video-content2", @"Get Video Content");
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            await CliRuntime.RunAsync(async () =>
            {
                ...
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);
                                ...
                                var response = await client.Videos.GetVideoContent2Async(
                                    videoId: videoId,
                                    variant: variant,
                                    xiApiKey: xiApiKey,
                                    xApiKey: xApiKey,
