...
internal static partial class AgentsRestoreBackupCommandApiCommand
{
    ...
    private static Argument<string> BackupId { get; } = new(
        name: @"backup-id")
    {
        Description = @"",
    };
    ...
    public static Command Create()
    {
        var command = new Command(@"restore-backup", @"Restore Backup");
                        ...
                        command.Arguments.Add(BackupId);
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            await CliRuntime.RunAsync(async () =>
            {
                        ...
                        var backupId = parseResult.GetRequiredValue(BackupId);
                        var xiApiKey = parseResult.GetValue(XiApiKey);
                        var xApiKey = parseResult.GetValue(XApiKey);
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);


                                var response = await client.Agents.RestoreBackupAsync(
